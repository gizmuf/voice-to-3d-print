...
secret_name_pattern='(^|_)(API_KEY|API_TOKEN|SECRET_KEY|ENCRYPTION_KEY|CLIENT_SECRET|ADMIN_TOKEN|TOKEN|EMAIL_ALLOWLIST)$'
failed=0
...
if jq -e '
  [.spec.template.spec.containers[].env[]?
   | select(.name | test("(^|_)(API_KEY|API_TOKEN|SECRET_KEY|ENCRYPTION_KEY|CLIENT_SECRET|ADMIN_TOKEN|TOKEN|EMAIL_ALLOWLIST)$"))]
  | length > 0
